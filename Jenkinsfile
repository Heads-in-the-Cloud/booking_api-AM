pipeline {
    ...
    environment {
        ...
        repo_name = 'am-bookings-api'
        jar_name = 'bookings-0.0.1-SNAPSHOT.jar'
            ...
            steps {
                ...
                withSonarQubeEnv('SonarQube') {
                    sh """
                       ...
                       -Dsonar.projectKey=AM-bookings-api \
        ...
        stage('ECS Update') {
            steps {
                ...
                dir("${AM_RESOURCES_DIRECTORY}") {
                    sh 'jq -M --arg commit "${commit}" \'.bookings=$commit\' images.json > tmp.$$.json && mv tmp.$$.json images.json'
                }
            }
        }
    }
